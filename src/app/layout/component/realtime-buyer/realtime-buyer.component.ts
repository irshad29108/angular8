...
@Component({
  selector: 'app-realtime-buyer',
  templateUrl: './realtime-buyer.component.html',
  styleUrls: ['./realtime-buyer.component.css']
})
export class RealtimeBuyerComponent implements OnInit {
  ...
  allRealtime() {
    ...
    this.service.getAllRealTimeBuyer(this.token).subscribe((res: any) => {
      var buyerName ='Unknown';
      this.loading = false;
      if(this.alluser.role=='admin'){

          this.data=res.map(({buyer_number,count,calls})=>{
          
          let data={
            buyer_number:buyer_number,
            count:count,
            calls:calls.map(({did,from_did,channel,talking_to,queue,buyer_number,duration,pub_id,pub_name,camp_name,buyer_id,status})=>{
        ...
        this.data=res.map(({buyer_number,count,calls})=>{
          ...
          let data={
            ...
            .map(({did,from_did,channel,talking_to,queue,buyer_number,duration,pub_id,pub_name,camp_name,buyer_id,status})=>{
